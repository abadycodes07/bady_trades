        ...
        <div className="container mx-auto max-w-[1500px] pt-4 pb-20 px-6 font-sans">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" className="bg-[var(--stats-card)] border-[var(--stats-card-border)] hover:bg-[var(--stats-card-hover)] text-[var(--foreground)]/80 hidden md:flex items-center gap-2 h-8 text-[11px] rounded-md">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 bg-[var(--stats-card)] rounded-md text-muted-foreground hover:text-[var(--foreground)] ml-2">
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-start">
                ...
                <div className="flex flex-col gap-4 flex-1 w-full min-w-0">
                    {daySummaries.length === 0 && (
                        <div className="text-center py-20 bg-[var(--stats-card)] rounded-xl border border-[var(--stats-card-border)]">
                            <p className="text-muted-foreground text-sm">No structured day data available.</p>
                        </div>
                    )}
                    ...
                    {daySummaries.map((day) => {
                        ...
                        return (
                            <div key={day.dateKey} className="bg-[var(--stats-card)] rounded-xl overflow-hidden shadow-sm border border-[var(--stats-card-border)] hover:border-indigo-500/20 transition-all">
                                ...
                                <div className="px-5 py-4 flex items-center justify-between border-b border-[var(--stats-card-border)] bg-[var(--stats-card)]">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => window.location.href = `/trades/${day.trades[0]?.id}`} className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-[var(--stats-card-hover)] hover:bg-[var(--stats-card-hover)]/80 border border-[var(--stats-card-border)] rounded-md text-[var(--foreground)]/80 text-[10px] font-bold tracking-wider transition-none">
                                            <PlayCircle className="h-3 w-3 text-indigo-400" /> Replay
                                        </button>
                                        <button onClick={() => window.location.href = `/trades/${day.trades[0]?.id}`} className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-[var(--stats-card-hover)] hover:bg-[var(--stats-card-hover)]/80 border border-[var(--stats-card-border)] rounded-md text-[var(--foreground)]/80 text-[10px] font-bold tracking-wider transition-none">
                                            <FileText className="h-3 w-3 text-muted-foreground" /> Add Note
                                        </button>
                                        <button onClick={() => window.location.href = `/trades/${day.trades[0]?.id}`} className="h-7 w-7 rounded-md bg-[var(--stats-card-hover)] border border-[var(--stats-card-border)] flex items-center justify-center ml-2">
                                ...
                                {/* Full Body */}
                                <div className="px-5 pt-3 pb-5 flex flex-col md:flex-row gap-8">
                                    {/* Left Side: Chart */}
                ...
                <div className="w-[300px] shrink-0 sticky top-6 hidden lg:block">
                    <div className="bg-[var(--stats-card)] rounded-xl border border-[var(--stats-card-border)] overflow-hidden shadow-2xl p-4">
                        <Calendar
                            mode="single"
                            ...
                            classNames={{
                                ...
                                caption: "flex justify-center pt-1 relative items-center mb-4 text-[var(--foreground)] font-bold",
                                caption_label: "text-sm",
                                nav: "space-x-1 flex items-center bg-[var(--stats-card-hover)] rounded-md",
                                nav_button: "h-7 w-7 bg-transparent hover:bg-[var(--foreground)]/10 text-[var(--foreground)] p-0 opacity-100 flex items-center justify-center rounded-md transition-none",
                                ...
                                day: "h-8 w-8 p-0 font-medium aria-selected:opacity-100 hover:bg-[var(--foreground)]/10 rounded-sm text-[var(--foreground)]/80 transition-none",
                                ...
                                day_today: "bg-[var(--foreground)]/10 text-[var(--foreground)] font-bold",
                                day_outside: "text-[var(--foreground)]/20 opacity-50",
