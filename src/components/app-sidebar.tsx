...
export function AppSidebar() {
  ...
  if (isCollapsed && !isMobile) {
    return (
      <TooltipProvider delayDuration={100}>
        <div
          className={cn(
            "fixed left-0 top-0 h-full z-40 flex flex-row border-r border-border bg-sidebar transition-all duration-300",
          )}
          style={{ width: '88px' }}
        >
          {/* LEFT COLUMN — utility icons */}
          <div className="flex flex-col items-center py-3 gap-1 w-10 flex-shrink-0">
            {/* Logo mark */}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  className="h-10 w-10 flex items-center justify-center rounded-xl bg-indigo-600/20 text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all mb-2 border border-indigo-500/20"
                  onClick={() => {
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="h-10 w-10 flex items-center justify-center rounded-xl text-muted-foreground/40 hover:bg-muted/20 hover:text-foreground transition-all mb-2"
                >
  ...
  return (
    <>
      ...
      <Sidebar side={isArabic ? "right" : "left"} variant="sidebar" collapsible="none" className="border-r border-border bg-sidebar transition-all duration-300 flex flex-col h-full">
        <SidebarHeader className="p-4 flex flex-col gap-3 border-b border-border">
        ...
        <SidebarContent className="flex flex-row flex-1 p-0 overflow-hidden">
          {/* LEFT UTILITY COLUMN */}
          <div className="flex flex-col items-center py-3 gap-1 w-12 border-r border-border flex-shrink-0">
            {leftNavItems.map((item) => (
          ...
          <div className="flex flex-col flex-1 overflow-y-auto py-2 px-2">
            ...
            <div className="mt-auto pt-2 border-t border-border space-y-1">
              <Link href="/settings" onClick={() => isMobile && setOpenMobile(false)}>
                <div className="flex items-center gap-3 px-3 py-2 rounded-xl text-muted-foreground/60 hover:text-foreground hover:bg-muted/10 transition-all cursor-pointer text-sm font-semibold">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <button className="relative flex items-center gap-3 px-3 py-2 rounded-xl text-muted-foreground/60 hover:text-foreground hover:bg-muted/10 transition-all w-full text-sm font-semibold">
        ...
        <SidebarFooter className="h-0 overflow-hidden p-0" />
      </Sidebar>
    </>
  );
}
